...
(defrecord Model [type permissions]
  ...
  (get-perm [this field role perm]
    (acl/get-perm permissions field role perm))
  (set-perm [this field role perm value]
    (assoc this :permissions
           (acl/set-perm permissions field role perm value))))

(defmacro apply-or
  "If kw is in (first args), call its value as a fn. Else, do body."
  [kw args & body]
  `(let [args# ~args
         f# (~kw (first args#))]
     (if f#
...
(defn get-model+ [this request doc]
  (get-model this request doc))
...
(def default-site-fns
  {:db-delete (fn [_ doc]
   ...
   :db-put (fn [_ doc]
             (mj/put-doc doc))
   :get-model (fn [this _ doc]
                (let [t (:type doc)]
                  (first (filter #(= t (:type %)) (:models this)))))
   :get-role (fn [model request & [doc]]
               (or (:chesty/api-role request)
                   (if-let [user (-> request :chesty/user kr/maybe-deref)]
                     (if (and doc (= (db/id user) (db/user-id doc)))
                       ...
                       :user)
                     :public)))
   :can? (fn [this request doc field perm]
           (boolean
...
(def site-validate-doc-fns
  {:validate (fn [this request old-doc new-doc]
               (validate (get-model+ this request (or old-doc new-doc)) request old-doc new-doc))})
...
(def site-doc-hooks-fns
  {:after-load (fn [this request doc]
                 (after-load (get-model+ this request doc) request doc))
   :before-save (fn [this request old-doc new-doc]
                  (before-save (get-model+ this request new-doc) request old-doc new-doc))
   :after-save (fn [this request old-doc new-doc]
...
(defn post-doc [site request]
  (let [doc (-> request :body-params ignore-keys)
    ...
    (if doc
      (errors-or (or (validate-can site request doc :chesty/model :create?)
